use std::{cell::RefCell, rc::Rc};

use crate::{
    Diagnostic, Span,
    ast::TypeKind,
    interpreter::{Interpreter, Value, iterable_values, values_equal},
    ir,
};

use super::builtin_method;
use crate::runtime::{RuntimeType, RuntimeTypeId};

pub(super) fn define() -> RuntimeType {
    RuntimeType {
        id: RuntimeTypeId(usize::MAX),
        ir_type_id: None,
        kind: TypeKind::Class,
        name: "Vector".to_string(),
        fields: Vec::new(),
        field_init: None,
        methods: vec![
            builtin_method(2, "append", vec![ir::Type::Unknown], list_append_mut),
            builtin_method(29, "add", vec![ir::Type::Unknown], list_append_mut),
            builtin_method(30, "addAll", vec![ir::Type::Unknown], list_add_all),
            builtin_method(3, "map", vec![function_unknown()], list_map),
            builtin_method(4, "flatMap", vec![function_unknown()], list_flat_map),
            builtin_method(5, "filter", vec![function_unknown()], list_filter),
            builtin_method(
                6,
                "fold",
                vec![ir::Type::Unknown, function_unknown()],
                list_fold,
            ),
            builtin_method(
                33,
                "reduce",
                vec![ir::Type::Unknown, function_unknown()],
                list_reduce_with_initial,
            ),
            builtin_method(7, "reduce", vec![function_unknown()], list_reduce),
            builtin_method(8, "exists", vec![function_unknown()], list_exists),
            builtin_method(9, "forEach", vec![function_unknown()], list_for_each),
            builtin_method(10, "forAll", vec![function_unknown()], list_for_all),
            builtin_method(11, "sort", vec![ir::Type::Unknown], list_sort),
            builtin_method(12, "zip", vec![ir::Type::Unknown], list_zip),
            builtin_method(13, "zipWithIndex", Vec::new(), list_zip_with_index),
            builtin_method(14, "size", Vec::new(), list_size),
            builtin_method(15, "isEmpty", Vec::new(), list_is_empty),
            builtin_method(31, "nonEmpty", Vec::new(), list_non_empty),
            builtin_method(32, "makeStr", vec![ir::Type::Str], list_make_str),
            builtin_method(16, "get", vec![ir::Type::Int], list_get),
            builtin_method(17, "remove", vec![ir::Type::Int], list_remove),
            builtin_method(34, "removeFirst", Vec::new(), list_remove_first),
            builtin_method(18, "removeLast", Vec::new(), list_remove_last),
            builtin_method(19, "head", Vec::new(), list_head),
            builtin_method(20, "tail", Vec::new(), list_tail),
            builtin_method(21, "first", Vec::new(), list_first),
            builtin_method(22, "last", Vec::new(), list_last),
            builtin_method(24, "count", vec![function_unknown()], list_count),
            builtin_method(25, "contains", vec![ir::Type::Unknown], list_contains),
            builtin_method(26, "find", vec![ir::Type::Unknown], list_find),
            builtin_method(27, "indexOf", vec![ir::Type::Unknown], list_index_of),
            builtin_method(28, "iterator", Vec::new(), list_iterator),
        ],
        enum_cases: Vec::new(),
        with_bounds: Vec::new(),
    }
}

fn function_unknown() -> ir::Type {
    ir::Type::Function {
        params: Vec::new(),
        ret: Box::new(ir::Type::Unknown),
    }
}

fn list_items(receiver: &Value) -> Rc<RefCell<Vec<Value>>> {
    let Value::List(items) = receiver else {
        unreachable!();
    };
    items.clone()
}

fn list_values(
    interpreter: &Interpreter<'_>,
    receiver: &Value,
    span: Option<Span>,
    context: &str,
) -> Result<Vec<Value>, Diagnostic> {
    let _ = (span, context);
    let items = list_items(receiver);
    let items = items.borrow();
    Ok(interpreter.clone_values(&items))
}

fn values_match(
    interpreter: &Interpreter<'_>,
    left: &Value,
    right: &Value,
    span: Option<Span>,
    context: &str,
) -> Result<bool, Diagnostic> {
    interpreter.ensure_observable_value(left, span, context)?;
    interpreter.ensure_observable_value(right, span, context)?;
    Ok(values_equal(left, right))
}

fn list_append_mut(
    interpreter: &mut Interpreter<'_>,
    receiver: Value,
    args: Vec<Value>,
    span: Option<Span>,
) -> Result<Value, Diagnostic> {
    if args.len() != 1 {
        return Err(interpreter.runtime_error(span, "Vector.append expects 1 argument"));
    }
    list_items(&receiver).borrow_mut().push(args[0].clone());
    Ok(receiver)
}

fn list_add_all(
    interpreter: &mut Interpreter<'_>,
    receiver: Value,
    args: Vec<Value>,
    span: Option<Span>,
) -> Result<Value, Diagnostic> {
    let [other] = args.as_slice() else {
        return Err(interpreter.runtime_error(span, "Vector.addAll expects 1 argument"));
    };
    let rhs = iterable_values(other.clone(), span, interpreter)?;
    list_items(&receiver).borrow_mut().extend(rhs);
    Ok(receiver)
}

fn list_map(
    interpreter: &mut Interpreter<'_>,
    receiver: Value,
    args: Vec<Value>,
    span: Option<Span>,
) -> Result<Value, Diagnostic> {
    let [callback] = args.as_slice() else {
        return Err(interpreter.runtime_error(span, "Vector.map expects 1 argument"));
    };
    let values = list_values(interpreter, &receiver, span, "Vector.map")?;
    let mut out = Vec::with_capacity(values.len());
    for value in values {
        out.push(interpreter.invoke_value(callback.clone(), vec![value], span)?);
    }
    Ok(Value::list(out))
}

fn list_flat_map(
    interpreter: &mut Interpreter<'_>,
    receiver: Value,
    args: Vec<Value>,
    span: Option<Span>,
) -> Result<Value, Diagnostic> {
    let [callback] = args.as_slice() else {
        return Err(interpreter.runtime_error(span, "Vector.flatMap expects 1 argument"));
    };
    let values = list_values(interpreter, &receiver, span, "Vector.flatMap")?;
    let mut out = Vec::new();
    for value in values {
        let mapped = interpreter.invoke_value(callback.clone(), vec![value], span)?;
        out.extend(iterable_values(mapped, span, interpreter)?);
    }
    Ok(Value::list(out))
}

fn list_filter(
    interpreter: &mut Interpreter<'_>,
    receiver: Value,
    args: Vec<Value>,
    span: Option<Span>,
) -> Result<Value, Diagnostic> {
    let [callback] = args.as_slice() else {
        return Err(interpreter.runtime_error(span, "Vector.filter expects 1 argument"));
    };
    let values = list_values(interpreter, &receiver, span, "Vector.filter")?;
    let mut out = Vec::new();
    for value in values {
        if interpreter
            .invoke_value(callback.clone(), vec![value.clone()], span)?
            .as_bool(interpreter, span, "Vector.filter predicate")?
        {
            out.push(value);
        }
    }
    Ok(Value::list(out))
}

fn list_fold(
    interpreter: &mut Interpreter<'_>,
    receiver: Value,
    args: Vec<Value>,
    span: Option<Span>,
) -> Result<Value, Diagnostic> {
    if args.len() != 2 {
        return Err(interpreter.runtime_error(span, "Vector.fold expects 2 arguments"));
    }
    let mut acc = args[0].clone();
    let callback = args[1].clone();
    let values = list_values(interpreter, &receiver, span, "Vector.fold")?;
    for value in values {
        acc = interpreter.invoke_value(callback.clone(), vec![acc, value], span)?;
    }
    Ok(acc)
}

fn list_reduce(
    interpreter: &mut Interpreter<'_>,
    receiver: Value,
    args: Vec<Value>,
    span: Option<Span>,
) -> Result<Value, Diagnostic> {
    let [callback] = args.as_slice() else {
        return Err(interpreter.runtime_error(span, "Vector.reduce expects 1 argument"));
    };
    let values = list_values(interpreter, &receiver, span, "Vector.reduce")?;
    let Some((first, rest)) = values.split_first() else {
        return Ok(interpreter.option_none());
    };
    let mut acc = first.clone();
    for value in rest {
        acc = interpreter.invoke_value(callback.clone(), vec![acc, value.clone()], span)?;
    }
    Ok(interpreter.option_some(acc))
}

fn list_reduce_with_initial(
    interpreter: &mut Interpreter<'_>,
    receiver: Value,
    args: Vec<Value>,
    span: Option<Span>,
) -> Result<Value, Diagnostic> {
    if args.len() != 2 {
        return Err(interpreter.runtime_error(span, "Vector.reduce expects 2 arguments"));
    }
    list_fold(interpreter, receiver, args, span)
}

fn list_exists(
    interpreter: &mut Interpreter<'_>,
    receiver: Value,
    args: Vec<Value>,
    span: Option<Span>,
) -> Result<Value, Diagnostic> {
    let [callback] = args.as_slice() else {
        return Err(interpreter.runtime_error(span, "Vector.exists expects 1 argument"));
    };
    let values = list_values(interpreter, &receiver, span, "Vector.exists")?;
    for value in values {
        if interpreter
            .invoke_value(callback.clone(), vec![value], span)?
            .as_bool(interpreter, span, "Vector.exists predicate")?
        {
            return Ok(Value::Bool(true));
        }
    }
    Ok(Value::Bool(false))
}

fn list_for_each(
    interpreter: &mut Interpreter<'_>,
    receiver: Value,
    args: Vec<Value>,
    span: Option<Span>,
) -> Result<Value, Diagnostic> {
    let [callback] = args.as_slice() else {
        return Err(interpreter.runtime_error(span, "Vector.forEach expects 1 argument"));
    };
    let values = list_values(interpreter, &receiver, span, "Vector.forEach")?;
    for value in values {
        let _ = interpreter.invoke_value(callback.clone(), vec![value], span)?;
    }
    Ok(Value::Unit)
}

fn list_for_all(
    interpreter: &mut Interpreter<'_>,
    receiver: Value,
    args: Vec<Value>,
    span: Option<Span>,
) -> Result<Value, Diagnostic> {
    let [callback] = args.as_slice() else {
        return Err(interpreter.runtime_error(span, "Vector.forAll expects 1 argument"));
    };
    let values = list_values(interpreter, &receiver, span, "Vector.forAll")?;
    for value in values {
        if !interpreter
            .invoke_value(callback.clone(), vec![value], span)?
            .as_bool(interpreter, span, "Vector.forAll predicate")?
        {
            return Ok(Value::Bool(false));
        }
    }
    Ok(Value::Bool(true))
}

fn list_sort(
    interpreter: &mut Interpreter<'_>,
    receiver: Value,
    args: Vec<Value>,
    span: Option<Span>,
) -> Result<Value, Diagnostic> {
    let [ordering] = args.as_slice() else {
        return Err(interpreter.runtime_error(span, "Vector.sort expects 1 argument"));
    };
    let items = list_items(&receiver);
    let mut values = {
        let items = items.borrow();
        interpreter.clone_values(&items)
    };
    let len = values.len();
    for i in 0..len {
        for j in (i + 1)..len {
            let cmp = interpreter.invoke_method(
                ordering.clone(),
                "compare",
                vec![values[i].clone(), values[j].clone()],
                span,
            )?;
            if cmp.as_int(interpreter, span, "Ordering.compare result")? > 0 {
                values.swap(i, j);
            }
        }
    }
    *items.borrow_mut() = values;
    Ok(receiver)
}

fn list_zip(
    interpreter: &mut Interpreter<'_>,
    receiver: Value,
    args: Vec<Value>,
    span: Option<Span>,
) -> Result<Value, Diagnostic> {
    let [other] = args.as_slice() else {
        return Err(interpreter.runtime_error(span, "Vector.zip expects 1 argument"));
    };
    let lhs = list_values(interpreter, &receiver, span, "Vector.zip")?;
    let rhs = iterable_values(other.clone(), span, interpreter)?;
    Ok(Value::list(
        lhs.into_iter()
            .zip(rhs)
            .map(|(left, right)| Value::Tuple(vec![left, right]))
            .collect(),
    ))
}

fn list_zip_with_index(
    interpreter: &mut Interpreter<'_>,
    receiver: Value,
    args: Vec<Value>,
    span: Option<Span>,
) -> Result<Value, Diagnostic> {
    if !args.is_empty() {
        return Err(interpreter.runtime_error(span, "Vector.zipWithIndex expects 0 arguments"));
    }
    let items = list_values(interpreter, &receiver, span, "Vector.zipWithIndex")?;
    Ok(Value::list(
        items
            .into_iter()
            .enumerate()
            .map(|(index, value)| Value::Tuple(vec![value, Value::Int(index as i64)]))
            .collect(),
    ))
}

fn list_size(
    interpreter: &mut Interpreter<'_>,
    receiver: Value,
    args: Vec<Value>,
    span: Option<Span>,
) -> Result<Value, Diagnostic> {
    if !args.is_empty() {
        return Err(interpreter.runtime_error(span, "Vector.size expects 0 arguments"));
    }
    Ok(Value::Int(list_items(&receiver).borrow().len() as i64))
}

fn list_is_empty(
    interpreter: &mut Interpreter<'_>,
    receiver: Value,
    args: Vec<Value>,
    span: Option<Span>,
) -> Result<Value, Diagnostic> {
    if !args.is_empty() {
        return Err(interpreter.runtime_error(span, "Vector.isEmpty expects 0 arguments"));
    }
    Ok(Value::Bool(list_items(&receiver).borrow().is_empty()))
}

fn list_non_empty(
    interpreter: &mut Interpreter<'_>,
    receiver: Value,
    args: Vec<Value>,
    span: Option<Span>,
) -> Result<Value, Diagnostic> {
    if !args.is_empty() {
        return Err(interpreter.runtime_error(span, "Vector.nonEmpty expects 0 arguments"));
    }
    Ok(Value::Bool(!list_items(&receiver).borrow().is_empty()))
}

fn list_make_str(
    interpreter: &mut Interpreter<'_>,
    receiver: Value,
    args: Vec<Value>,
    span: Option<Span>,
) -> Result<Value, Diagnostic> {
    let [separator] = args.as_slice() else {
        return Err(interpreter.runtime_error(span, "Vector.makeStr expects 1 argument"));
    };
    let Value::String(separator) = separator else {
        return Err(interpreter.runtime_error(
            span,
            format!(
                "Vector.makeStr expects Str separator, got {}",
                separator.render()
            ),
        ));
    };

    let items = list_values(interpreter, &receiver, span, "Vector.makeStr")?;
    let mut rendered = String::new();
    for (index, item) in items.iter().enumerate() {
        if index > 0 {
            rendered.push_str(separator);
        }
        rendered.push_str(&interpreter.render_value(item, span, "Vector.makeStr")?);
    }
    Ok(Value::String(rendered))
}

fn list_get(
    interpreter: &mut Interpreter<'_>,
    receiver: Value,
    args: Vec<Value>,
    span: Option<Span>,
) -> Result<Value, Diagnostic> {
    if args.len() != 1 {
        return Err(interpreter.runtime_error(span, "Vector.get expects 1 argument"));
    }
    let index = args[0].as_int(interpreter, span, "Vector.get index")?;
    let items = list_items(&receiver);
    let items = items.borrow();
    let value = items
        .get(index as usize)
        .map(|value| interpreter.clone_value(value));
    Ok(match value {
        Some(value) => interpreter.option_some(value),
        None => interpreter.option_none(),
    })
}

fn list_remove(
    interpreter: &mut Interpreter<'_>,
    receiver: Value,
    args: Vec<Value>,
    span: Option<Span>,
) -> Result<Value, Diagnostic> {
    if args.len() != 1 {
        return Err(interpreter.runtime_error(span, "Vector.remove expects 1 argument"));
    }
    let index = args[0].as_int(interpreter, span, "Vector.remove index")?;
    let items = list_items(&receiver);
    let mut items = items.borrow_mut();
    if index < 0 || index as usize >= items.len() {
        return Ok(interpreter.option_none());
    }
    let value = items.remove(index as usize);
    Ok(interpreter.option_some(value))
}

fn list_remove_first(
    interpreter: &mut Interpreter<'_>,
    receiver: Value,
    args: Vec<Value>,
    span: Option<Span>,
) -> Result<Value, Diagnostic> {
    if !args.is_empty() {
        return Err(interpreter.runtime_error(span, "Vector.removeFirst expects 0 arguments"));
    }
    let items = list_items(&receiver);
    let mut items = items.borrow_mut();
    if items.is_empty() {
        return Ok(interpreter.option_none());
    }
    let value = items.remove(0);
    Ok(interpreter.option_some(value))
}

fn list_remove_last(
    interpreter: &mut Interpreter<'_>,
    receiver: Value,
    args: Vec<Value>,
    span: Option<Span>,
) -> Result<Value, Diagnostic> {
    if !args.is_empty() {
        return Err(interpreter.runtime_error(span, "Vector.removeLast expects 0 arguments"));
    }
    let value = list_items(&receiver).borrow_mut().pop();
    Ok(match value {
        Some(value) => interpreter.option_some(value),
        None => interpreter.option_none(),
    })
}

fn list_head(
    interpreter: &mut Interpreter<'_>,
    receiver: Value,
    args: Vec<Value>,
    span: Option<Span>,
) -> Result<Value, Diagnostic> {
    if !args.is_empty() {
        return Err(interpreter.runtime_error(span, "Vector.head expects 0 arguments"));
    }
    let items = list_items(&receiver);
    let items = items.borrow();
    let value = items.first().map(|value| interpreter.clone_value(value));
    Ok(match value {
        Some(value) => interpreter.option_some(value),
        None => interpreter.option_none(),
    })
}

fn list_tail(
    interpreter: &mut Interpreter<'_>,
    receiver: Value,
    args: Vec<Value>,
    span: Option<Span>,
) -> Result<Value, Diagnostic> {
    if !args.is_empty() {
        return Err(interpreter.runtime_error(span, "Vector.tail expects 0 arguments"));
    }
    let items = list_items(&receiver);
    let values = items.borrow();
    let tail = if values.len() <= 1 {
        Vec::new()
    } else {
        interpreter.clone_values(&values[1..])
    };
    Ok(Value::list(tail))
}

fn list_first(
    interpreter: &mut Interpreter<'_>,
    receiver: Value,
    args: Vec<Value>,
    span: Option<Span>,
) -> Result<Value, Diagnostic> {
    if !args.is_empty() {
        return Err(interpreter.runtime_error(span, "Vector.first expects 0 arguments"));
    }
    let items = list_items(&receiver);
    let items = items.borrow();
    let value = items.first().map(|value| interpreter.clone_value(value));
    Ok(match value {
        Some(value) => interpreter.option_some(value),
        None => interpreter.option_none(),
    })
}

fn list_last(
    interpreter: &mut Interpreter<'_>,
    receiver: Value,
    args: Vec<Value>,
    span: Option<Span>,
) -> Result<Value, Diagnostic> {
    if !args.is_empty() {
        return Err(interpreter.runtime_error(span, "Vector.last expects 0 arguments"));
    }
    let items = list_items(&receiver);
    let items = items.borrow();
    let value = items.last().map(|value| interpreter.clone_value(value));
    Ok(match value {
        Some(value) => interpreter.option_some(value),
        None => interpreter.option_none(),
    })
}

fn list_count(
    interpreter: &mut Interpreter<'_>,
    receiver: Value,
    args: Vec<Value>,
    span: Option<Span>,
) -> Result<Value, Diagnostic> {
    let [callback] = args.as_slice() else {
        return Err(interpreter.runtime_error(span, "Vector.count expects 1 argument"));
    };
    let values = list_values(interpreter, &receiver, span, "Vector.count")?;
    let mut count = 0i64;
    for value in values {
        if interpreter
            .invoke_value(callback.clone(), vec![value], span)?
            .as_bool(interpreter, span, "Vector.count predicate")?
        {
            count += 1;
        }
    }
    Ok(Value::Int(count))
}

fn list_contains(
    interpreter: &mut Interpreter<'_>,
    receiver: Value,
    args: Vec<Value>,
    span: Option<Span>,
) -> Result<Value, Diagnostic> {
    let [needle] = args.as_slice() else {
        return Err(interpreter.runtime_error(span, "Vector.contains expects 1 argument"));
    };
    let items = list_items(&receiver);
    for value in items.borrow().iter() {
        if values_match(interpreter, value, needle, span, "Vector.contains")? {
            return Ok(Value::Bool(true));
        }
    }
    Ok(Value::Bool(false))
}

fn list_find(
    interpreter: &mut Interpreter<'_>,
    receiver: Value,
    args: Vec<Value>,
    span: Option<Span>,
) -> Result<Value, Diagnostic> {
    let [needle] = args.as_slice() else {
        return Err(interpreter.runtime_error(span, "Vector.find expects 1 argument"));
    };
    let items = list_items(&receiver);
    for value in items.borrow().iter() {
        if values_match(interpreter, value, needle, span, "Vector.find")? {
            return Ok(interpreter.option_some(value.clone()));
        }
    }
    Ok(interpreter.option_none())
}

fn list_index_of(
    interpreter: &mut Interpreter<'_>,
    receiver: Value,
    args: Vec<Value>,
    span: Option<Span>,
) -> Result<Value, Diagnostic> {
    let [needle] = args.as_slice() else {
        return Err(interpreter.runtime_error(span, "Vector.indexOf expects 1 argument"));
    };
    let items = list_items(&receiver);
    for (index, value) in items.borrow().iter().enumerate() {
        if values_match(interpreter, value, needle, span, "Vector.indexOf")? {
            return Ok(Value::Int(index as i64));
        }
    }
    Ok(Value::Int(-1))
}

fn list_iterator(
    interpreter: &mut Interpreter<'_>,
    receiver: Value,
    args: Vec<Value>,
    span: Option<Span>,
) -> Result<Value, Diagnostic> {
    if !args.is_empty() {
        return Err(interpreter.runtime_error(span, "Vector.iterator expects 0 arguments"));
    }
    Ok(Value::iterator_from_values(
        list_items(&receiver).borrow().clone(),
    ))
}
